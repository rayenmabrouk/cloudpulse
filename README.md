...
Everything in the AWS box is created by Terraform, except two S3 buckets and the IAM instance profile (pre-created by AWS Academy). The buckets are created by a bootstrap script: the state bucket because Terraform cannot store its state in a bucket it has not created yet, and the backup bucket because of an AWS Academy restriction (see [limitations](#aws-academy-limitations)); Terraform still manages all of the backup bucket's settings.
...
| Operations docs | Runbook, troubleshooting (17 real issues), cost analysis | `docs/` |
...
| **S3 (backups)** | Daily SQLite backups: versioned, SSE-S3, TLS-only, public access blocked, 14-day expiry (all configured by Terraform; the bucket itself is bootstrapped). The EBS volume is otherwise the only copy of the data. |
...
- **Gates:** Trivy blocks any fixable CRITICAL/HIGH vulnerability in CI and CD; Checkov (40 passed, 0 failed, 13 skipped, each skip justified inline) and TFLint gate Terraform; hadolint gates the Dockerfile; infrastructure changes need my approval before apply; `master` is branch-protected.
...
To stop all charges: `terraform -chdir=terraform destroy` (ECR images are force-deleted; the state and backup buckets are removed by hand, see the runbook).
...
| Service control policy denies `s3:GetBucketObjectLockConfiguration` | The AWS provider cannot manage an `aws_s3_bucket` resource (every read fails), so the backup bucket is created by the bootstrap script and only its configuration is in Terraform | A normal `aws_s3_bucket` resource |
...
│   ├── bootstrap-tfstate.ps1  # one-time creation of the state and backup buckets
